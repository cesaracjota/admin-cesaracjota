...
const GlobalFilter = ({ globalFilter, setGlobalFilter, data }) => {
    const csvData = [
        ["ID", "USUARIO", "CORREO", "TELEFONO", "FECHA ENVIO"],
        ...data.map(({ _id, nombres_usuario, telefono, email, createdAt }) => [
            _id,
            nombres_usuario,
            telefono,
            email,
...
const TablaMensajes = ({ columns, data }) => {
    const {
        getTableProps,
        getTableBodyProps,
        headerGroups,
...
const isEstadoAccionesColumn = (column) => {
    return column.id === 'estado' || column.id === 'acciones' || column.id === 'role';
};
